...
# Utility function to save paths to a text file
...
def retrieve_image_video_files_from_blob_storage(container_name, directory_prefix):
    ...
    for blob in blobs:
        if blob.name.endswith('.png') or blob.name.endswith('.mp4'):
            # Ensure the blob path starts with a '/'
...
def load_paths_from_file(file_path):
    """Loads all file paths from a text file into a list."""
    with open(file_path, 'r') as file:
...
def compare_paths(json_src_paths, blob_src_paths):
    ...
    json_src_paths_set = set([path.lower() for path in json_src_paths])
    blob_src_paths_set = set([path.lower() for path in blob_src_paths])
